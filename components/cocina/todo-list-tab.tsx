...
export function TodoListTab() {
  ...
  const renderCard = (card: any) => (
    <div
      ...
      className="bg-background cursor-grab active:cursor-grabbing border rounded-lg p-4 shadow-sm flex flex-col gap-3 hover:border-[#FAD85D] transition-colors"
    >
      <div className="flex justify-between items-start">
        <p className="font-semibold text-sm text-[#4A3B32]">{card.userName}</p>
        <span className="text-xs font-medium text-[#877669] bg-[#FAD85D]/40 px-2 py-0.5 rounded">{card.time}</span>
      </div>
      <ul className="space-y-1.5">
        {card.items.map((item: string, idx: number) => (
          <li key={idx} className="text-xs text-[#877669]">
            • {item}
          </li>
        ))}
      </ul>
      {card.status !== "completed" ? (
        <button
          ...
          className="mt-2 text-xs font-medium w-full bg-[#FFFFFF] border border-[#FAD85D] hover:bg-[#FAD85D]/50 text-[#4A3B32] py-2 rounded transition-colors"
        >
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full min-h-[600px] items-start">
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="font-bold text-[#4A3B32]">Pendientes</h3>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="font-bold text-[#4A3B32]">Preparando</h3>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h3 className="font-bold text-[#4A3B32]">Completados</h3>
