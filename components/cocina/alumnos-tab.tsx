    ...
    <Card className="border-[#FAD85D] bg-[#FFFFFF] shadow-sm">
      <CardHeader className="pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <CardTitle className="text-2xl text-[#4A3B32]">Gestión de <span className="text-[#E8654D]">Alumnos</span></CardTitle>
            <CardDescription className="text-[#877669]">Administra los permisos de tus estudiantes de cocina</CardDescription>
          </div>
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent className="bg-[#FFFFFF] border-[#FAD85D]">
              <DialogHeader>
                <DialogTitle className="text-2xl text-[#4A3B32]">Nuevo <span className="text-[#E8654D]">Alumno</span></DialogTitle>
              ...
              <form onSubmit={handleAddUser} className="space-y-4 py-4">
                <div className="space-y-2">
                  <Label htmlFor="add-name" className="text-[#4A3B32]">Nombre Completo</Label>
                  <Input 
                    id="add-name" 
                    value={newUser.name} 
                ...
                <div className="space-y-2">
                  <Label htmlFor="add-email" className="text-[#4A3B32]">Correo Electrónico</Label>
                  <Input 
                    id="add-email" 
                    type="email"
                    value={newUser.email} 
                ...
                <div className="space-y-2">
                  <Label htmlFor="add-password" className="text-[#4A3B32]">Contraseña Inicial</Label>
                  <Input 
                    id="add-password" 
                    type="password"
                ...
                <DialogFooter className="pt-4">
                  <Button type="button" variant="outline" onClick={() => setIsAddDialogOpen(false)} className="border-[#FAD85D] text-[#877669] hover:bg-[#FAD85D]/50">
                    Cancelar
                  </Button>
      ...
      <CardContent>
        {alumnos.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center border rounded-lg border-dashed border-[#FAD85D]">
            <Users className="h-12 w-12 text-[#FAD85D] mb-4" />
            <h3 className="text-lg font-medium text-[#4A3B32]">Aún no hay alumnos</h3>
            <p className="text-[#877669] mt-1">Añade a tus estudiantes para que puedan ayudarte en la cocina.</p>
          </div>
        ) : (
            ...
            <Table className="min-w-[600px] sm:min-w-0">
              <TableHeader className="bg-[#FFFFFF]">
                <TableRow className="hover:bg-transparent border-[#FAD85D]/50">
                  ...
                  <TableHead className="font-bold text-[#4A3B32] hidden md:table-cell">Email</TableHead>
                  <TableHead className="font-bold text-[#4A3B32]">Rol</TableHead>
                  <TableHead className="font-bold text-[#4A3B32]">Valoración Pública</TableHead>
                  <TableHead className="font-bold text-[#4A3B32] text-right">Acciones</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {alumnos.map((alumno) => (
                  <TableRow key={alumno.id} className="border-[#FAD85D]/30 hover:bg-[#FDF1B6]/10 transition-colors">
                    <TableCell className="font-medium text-[#4A3B32]">{alumno.name}</TableCell>
                    <TableCell className="text-[#877669]">{alumno.email}</TableCell>
                    <TableCell>
                      <Badge className={alumno.role === 'alumno-cocina-titular' ? "bg-[#FAD85D] text-[#877669] shadow-none border-none" : "bg-[#F0F1F2] text-[#877669] shadow-none border-none"}>
                    ...
                    <TableCell>
                      {(() => {
                        ...
                        if (!rating) return <span className="text-xs text-[#877669] italic">Sin valoraciones</span>
                        return (
                          <div className="flex items-center gap-1.5">
                            <Star className="h-4 w-4 fill-[#FAD85D] text-[#FAD85D]" />
                            <span className="font-medium text-[#4A3B32]">{rating.avg.toFixed(1)}</span>
                            <span className="text-xs text-[#877669]">({rating.count})</span>
                          </div>
                        )
                      })()}
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        >
                          <DialogTrigger asChild>
                            <Button size="sm" variant="outline" className="h-8 w-8 p-0 border-[#FAD85D] text-[#877669] hover:bg-[#FAD85D]/50">
                          ...
                          {editingUser?.id === alumno.id && (
                            <DialogContent className="bg-[#FFFFFF] border-[#FAD85D]">
                              <DialogHeader>
                                <DialogTitle className="text-2xl text-[#4A3B32]">Editar <span className="text-[#E8654D]">Alumno</span></DialogTitle>
                                <DialogDescription className="text-[#877669]">
                              ...
                              <form onSubmit={handleSaveEdit} className="space-y-4 py-4">
                                <div className="space-y-2">
                                  <Label htmlFor="edit-name" className="text-[#4A3B32]">Nombre Completo</Label>
                                  <Input 
                                    id="edit-name" 
                                ...
                                <div className="space-y-2">
                                  <Label htmlFor="edit-email" className="text-[#4A3B32]">Correo Electrónico</Label>
                                  <Input 
                                    id="edit-email" 
                                    type="email"
                                ...
                                <div className="space-y-2">
                                  <Label htmlFor="edit-role" className="text-[#4A3B32]">Permisos (Rol)</Label>
                                  <Select 
                                    value={editingUser.role} 
                                    onValueChange={(value) => setEditingUser(prev => prev ? { ...prev, role: value } : null)}
                                  >
                                    <SelectTrigger id="edit-role" className="border-[#FAD85D] focus:ring-[#E8654D]/20">
                                ...
                                <div className="space-y-2">
                                  <Label htmlFor="edit-password" className="text-[#4A3B32]">Nueva Contraseña</Label>
                                  <Input 
                                    id="edit-password" 
                                    type="password"
                                ...
                                <DialogFooter className="pt-4">
                                  <Button type="button" variant="outline" onClick={() => setIsEditDialogOpen(false)} className="border-[#FAD85D] text-[#877669] hover:bg-[#FAD85D]/50">
                                    Cancelar
                                  </Button>
                                  <Button type="submit" className="bg-[#FAD85D] text-[#4A3B32] hover:bg-[#FDF1B6]">
                                    Guardar Cambios
                          ...
                          <AlertDialogContent className="bg-[#FFFFFF] border-[#FAD85D]">
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription className="text-[#877669]">
                                Esta acción retirará el acceso a la cocina para <span className="font-bold text-[#4A3B32]">{alumno.name}</span>.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
